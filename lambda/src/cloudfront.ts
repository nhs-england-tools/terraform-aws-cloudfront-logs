...
export const parseVersion: ParseVersionFunction = (line) => {
  if (line === undefined) {
    throw new Error('Empty version line')
  }
...
export const parseFields: ParseFieldsFunction = (line) => {
  if (line === undefined) {
    throw new Error('Empty fields line')
  }
...
interface ParseLogFileOptions {
  bucket: string
  key: string
  region: string
}

type ParseLogFileFunction = (options: ParseLogFileOptions) => Promise<Array<Record<string, string>>>
